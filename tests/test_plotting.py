...
from probfit.functor import AddPdf, AddPdfNorm, Extended
from probfit.funcutil import rename
from probfit.pdf import gaussian, linear
from probfit.plotting import draw_compare_hist, draw_pdf
...
def test_draw_residual_ulh_norm():
    ...
    data = np.random.randn(1000)
    ulh = UnbinnedLH(gaussian, data)
    ulh.draw_residual(args=(0.0, 1.0), norm=True)
    plt.ylim(-7.0, 3.0)
    plt.xlim(-4.0, 3.0)


@image_comparison("draw_residual_ulh_norm_no_errbars.png", tolerance=special_tol)
def test_draw_residual_ulh_norm_no_errbars():
    np.random.seed(0)
    data = np.random.randn(1000)
    ulh = UnbinnedLH(gaussian, data)
    ulh.draw_residual(args=(0.0, 1.0), norm=True, show_errbars=False)
...
def test_draw_ulh_extend_residual_norm():
    ...
    data = np.random.randn(1000)
    ulh = UnbinnedLH(Extended(gaussian), data, extended=True)
    ulh.draw_residual(args=(0.0, 1.0, 1000), norm=True)
    plt.ylim(-7.0, 3.0)
...
def test_draw_residual_blh():
    ...
    data = np.random.randn(1000)
    blh = BinnedLH(gaussian, data)
    blh.draw_residual(args=(0.0, 1.0))
...
def test_draw_residual_blh_norm():
    ...
    data = np.random.randn(1000)
    blh = BinnedLH(gaussian, data)
    blh.draw_residual(args=(0.0, 1.0), norm=True)
    plt.ylim(-4.0, 3.0)
    plt.xlim(-4.0, 3.0)
...
def test_draw_residual_blh_norm_no_errbars():
    np.random.seed(0)
    data = np.random.randn(1000)
    blh = BinnedLH(gaussian, data)
    blh.draw_residual(args=(0.0, 1.0), norm=True, show_errbars=False)
...
def test_draw_blh_extend_residual_norm():
    ...
    data = np.random.randn(1000)
    blh = BinnedLH(Extended(gaussian), data, extended=True)
    blh.draw_residual(args=(0.0, 1.0, 1000), norm=True)
...
def test_draw_x2reg():
    ...
    y = 10.0 * x + np.random.randn(100)
    err = np.array([1] * 100)
    blh = Chi2Regression(linear, x, y, err)
    blh.draw(args=(10.0, 0.0))
