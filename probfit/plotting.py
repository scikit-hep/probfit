...
from .util import describe, parse_arg
...
def draw_ulh(
    ...
    if not show_errbars:
        ...
        if not no_plot:
            ax.errorbar(mid(e), n, np.sqrt(w2), fmt="b.", capsize=0, zorder=0)
    ...
    total_ret = draw_pdf_with_edges(
        self.f, arg, ef, ax=ax, density=not self.extended, scale=scale, **dict(draw_arg)
    )
    ...
    if not no_plot and print_par:
        ax.text(
            parmloc[0], parmloc[1], txt, ha="left", va="top", transform=ax.transAxes
        )
    return (data_ret, error_ret, total_ret, part_ret)


def draw_residual(
    x, y, yerr, xerr, show_errbars=True, ax=None, zero_line=True, grid=True, **kwargs
):
    ...
    **Returns**
    ...
    if show_errbars:
        ...
        ax.errorbar(x, y, yerr, xerr, zorder=0, **plotopts)
    else:
        plotopts = dict(color="k")
        plotopts.update(kwargs)
        ax.bar(x - xerr, y, width=2 * xerr, **plotopts)
...
def draw_x2(
    ...
    if data_err is None:
        ...
        if not no_plot:
            ax.errorbar(x, y, data_err, fmt=".", zorder=0)
    ...
    total_ret = draw_pdf_with_midpoints(
        self.f, arg, x, no_plot=no_plot, ax=ax, **dict(draw_arg)
    )
    ...
    if parts:
        ...
        if f_parts is not None:
            for p in f_parts():
                tmp = draw_pdf_with_midpoints(
                    p, arg, x, ax=ax, no_plot=no_plot, **dict(draw_arg)
                )
                part_ret.append(tmp)
...
def draw_x2_residual(
    self, minuit=None, ax=None, args=None, errors=None, grid=True, norm=False
):
    from matplotlib import pyplot as plt
...
def draw_compare(
    f, arg, edges, data, errors=None, ax=None, grid=True, normed=False, parts=False
):
    """
    ...
    total = np.sum(data)
    if normed:
        ax.errorbar(x, data / bw / total, errors / bw / total, fmt=".b", zorder=0)
    ...
    if parts:
        if not hasattr(f, "eval_parts"):
            warn(
                RuntimeWarning(
                    "parts is set to True but function does "
                    "not have eval_parts method"
                )
            )
        else:
            scale = bw if not normed else 1.0
            parts_val = list()
            for tx in x:
...
def draw_normed_pdf(f, arg, bound, bins=100, scale=1.0, density=True, ax=None, **kwds):
    return draw_pdf(
        f, arg, bound, bins=100, scale=1.0, density=True, normed_pdf=True, ax=ax, **kwds
    )


def draw_pdf(
    f, arg, bound, bins=100, scale=1.0, density=True, normed_pdf=False, ax=None, **kwds
):
    """
...
def draw_pdf_with_edges(
    f, arg, edges, ax=None, scale=1.0, density=True, normed_pdf=False, **kwds
):
    x = (edges[:-1] + edges[1:]) / 2.0
    bw = np.diff(edges)
    scale *= bw if not density else 1.0

    return draw_pdf_with_midpoints(
        f, arg, x, ax=ax, scale=scale, normed_pdf=normed_pdf, **kwds
    )


def draw_pdf_with_midpoints(
    f, arg, x, ax=None, scale=1.0, normed_pdf=False, no_plot=False, **kwds
):
    from matplotlib import pyplot as plt
